describe Dumagst::Engines::MinhashEngine do
  ...
  describe "#recommend_users" do
    context "with a native matrix" do
      ...
      subject { Dumagst::Engines::MinhashEngine.new(matrix: matrix, similarity_threshold: 0.88, buckets: 9)}
  ...
  describe "#recommend_products" do
    context "with a native matrix" do
      before(:each) { Dumagst.configuration.redis_connection.flushdb }
      let(:matrix) { Dumagst::Matrices::NativeMatrix.from_csv(csv_file, 9, 8) }
      subject { Dumagst::Engines::MinhashEngine.new(matrix: matrix, similarity_threshold: 0.2, buckets: 9)}
      it "returns an empty array if there are no matches" do
        subject.process
        expect(subject.recommend_products(22, false)).to eq([])
      end
      xit "returns a set of similar products that the user might like given the user ID, excluding user's own likes" do
        subject.process
      ...
      xit "returns the set of similar products with scores by default" do
        subject.process
  ...
  describe "#users_with_recommended_products" do
    context "with a native matrix" do
      before(:each) { Dumagst.configuration.redis_connection.flushdb }
      let(:matrix) { Dumagst::Matrices::NativeMatrix.from_csv(csv_file, 9, 8) }
      subject { Dumagst::Engines::MinhashEngine.new(matrix: matrix, similarity_threshold: 0.4, buckets: 9)}
      xit "returns the ids of the users that have recommended products" do
        subject.process
